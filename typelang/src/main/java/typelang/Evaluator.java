...
public class Evaluator implements Visitor<Value, Env<Value>> {
  ...
  public Value visit(CallExp e, Env<Value> env) { // New for funclang.
    ...
    if (!(result instanceof Value.FunVal))
      return new Value.DynamicError(
          "Operator not a function in call " + ts.visit(e, null));
  ...
  private Env<Value> appendEnv(Env<Value> fst, Env<Value> snd) {
    ...
    if (fst instanceof ExtendEnv) {
      ...
      return new ExtendEnv<>(appendEnv(f.saved_env(), snd), f.var(),
          f.val());
    }
    ExtendEnvRec f = (ExtendEnvRec) fst;
    return new ExtendEnvRec(appendEnv(f.saved_env(), snd), f.names(),
        f.vals());
  }
  ...
  public Value visit(IfExp e, Env<Value> env) { // New for funclang.
    ...
    if (!(result instanceof Value.BoolVal))
      return new Value.DynamicError(
          "Condition not a boolean in expression "
              + ts.visit(e, null));
  ...
  public Value visit(ReadExp e, Env<Value> env) {
    ...
    try {
      String text = Reader.readFile("" + System.getProperty("user.dir")
  ...
  public Value visit(IsListExp e, Env<Value> env) {
    ...
    return new BoolVal(
        val instanceof Value.PairVal && ((Value.PairVal) val).isList()
  ...
  private Env<Value> initialEnv() {
    ...
    /*
     * Procedure: (read <filename>). Following is same as (define read
    ...
    /*
     * Procedure: (require <filename>). Following is same as (define require
     * (lambda (file) (eval (read file))))
     */
